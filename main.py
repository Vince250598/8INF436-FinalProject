...
from HierarchicalClustering import trainAndTestHierModel
from DBSCAN import trainAndTestDBSCANModel

from KMeans import trainAndTestKmeansModel
...
max_iter = 300
tol = 0.0001
kmeansResults = pd.DataFrame(
    columns=['average_precision', 'kappa_score', 'weighted_precision', 'weighted_recall', 'weighted_f1_score',
             'matthews_correlation_coefficient'])

for random_state in range(2020, 2023):
    for i in range(0, 3):
        kmeansResults.loc[len(kmeansResults)] = trainAndTestKmeansModel(data, random_state, n_clusters, n_init,
                                                                        max_iter, tol)

kmeansResultsMeanValues = kmeansResults.mean()
kmeansResultsCSV = pd.read_csv('kmeansResults.csv')

kmeansResultsCSV.loc[len(kmeansResultsCSV)] = [n_clusters, n_init, max_iter, tol,
                                               kmeansResultsMeanValues.average_precision,
                                               kmeansResultsMeanValues.kappa_score,
                                               kmeansResultsMeanValues.weighted_precision,
                                               kmeansResultsMeanValues.weighted_recall,
                                               kmeansResultsMeanValues.weighted_f1_score,
...
hierResultsCSV.loc[len(hierResultsCSV)] = [n_clusters, hierResultsMeanValues.average_precision,
                                           ...
                                           hierResultsMeanValues.weighted_recall,
                                           hierResultsMeanValues.weighted_f1_score,
...
DBSCANResultsCSV.loc[len(DBSCANResultsCSV)] = [eps, min_samples, DBSCANResultsMeanValues.average_precision,
                                               DBSCANResultsMeanValues.kappa_score,
                                               DBSCANResultsMeanValues.weighted_precision,
                                               DBSCANResultsMeanValues.weighted_recall,
                                               DBSCANResultsMeanValues.weighted_f1_score,
